...
cd ~/dev_ws/src/stretch_web_teleop && pm2 start -s npm --name="stretch_web_teleop" -- run $STORAGE &>> $REDIRECT_LOGFILE

echo "Start local server..."
cd ~/dev_ws/src/stretch_web_teleop && pm2 start -s server.js --watch &>> $REDIRECT_LOGFILE

echo "Start robot browser..."
cd ~/dev_ws/src/stretch_web_teleop && pm2 start -s start_robot_browser.js --watch &>> $REDIRECT_LOGFILE
