...
export const Tab = (props: {
    ...
    return (
        props.pill 
        ? <li className={className('pill-tab-btn', {active})} onClick={props.onClick} key={props.label}>{props.label}</li>
        : <button className={className('tab-btn', {active})} onClick={props.onClick} key={props.label}>{props.label}</button>
    )
}
            ...
            <div className={props.pill ? 'pill-tab' : 'tab'}>
                {tabLabels.map((label, index) => (
                    <Tab
                        ...
                        onClick={() => { props.onChange(index); setActiveIndex(index) }}
                        pill={props.pill}
                    />
                ))}
            </div>
