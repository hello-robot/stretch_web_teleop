...
import {
  RobotPose,
  ValidJointStateDict,
  BatteryVoltageMessage, // Interfaces
  statustoRobotPose,
  statustoInJointLimits,
  statustoInCollision,
  statustoBatteryVoltageMessage,
  disallowConcurrency, // Methods
...
export class Robot {
  ...
  private headNavCamStreamUncompressAndSendCallback: (
    compressed_image: Buffer,
  ) => void;
  syncStopTrajectory;
  ...
  constructor(
    ...
    headNavCamStreamUncompressAndSendCallback: (
      compressed_image: Buffer,
    ) => void,
  ) {
    ...
    this.headNavCamStreamUncompressAndSendCallback =
      headNavCamStreamUncompressAndSendCallback;
  }
  ...
  async connect(): Promise<void> {
    ...
    try {
      const config = yaml.load(
        fs.readFileSync(`${os.homedir()}/.stretch/config.yaml`, "utf8"),
      );
      ip_addr = config.robots[0].ip_addr;
      port = config.robots[0].port;
    } catch (error) {
      console.error(
        "Failed to connect to Stretch. Confirm ~/.stretch/config.yaml exists.",
      );
    }
    ...
    (async function () {
      for await (const [status_buff] of this.status_sock) {
        ...
        let batteryState: BatteryVoltageMessage =
          statustoBatteryVoltageMessage(status);
        if (this.batteryStateCallback) {
          this.batteryStateCallback(batteryState);
        }

        // runstop state
        if (this.isRunStoppedCallback) {
          this.isRunStoppedCallback(status["other"]["is_runstopped"]);
        }
      }
    }).call(this);
  }
  ...
  subscribeToHeadNavCam() {
    (async function () {
      for await (const [hncb64_buff] of this.hncb64_sock) {
        if (this.headNavCamStreamUncompressAndSendCallback) {
          this.headNavCamStreamUncompressAndSendCallback(hncb64_buff);
        }
