import { CameraInfo, SignallingMessage, WebRTCMessage } from "./util_node";
import io, { Socket } from "socket.io-client";
import { safelyParseJSON, generateUUID } from "./util_node";
import {
  RTCPeerConnection,
...
export class WebRTCConnection {
  ...
  constructor(props: WebRTCProps) {
    ...
    this.socket.on("signalling", (message: SignallingMessage) => {
      ...
      if (
        ...
        if (this.isSettingRemoteAnswerPending == true) return;
        this.peerConnection
  ...
  addTrackWithoutStream(track: MediaStreamTrack, streamName: string) {
    this.cameraInfo[track.id] = streamName;
    if (!this.peerConnection) throw "pc is undefined";
    this.peerConnection.addTrack(track);
    console.log("added track without stream");
  }
