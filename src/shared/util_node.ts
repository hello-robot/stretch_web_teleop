...
export function statustoInJointLimits(status): ValidJointStateDict {
    ...
    Object.keys(status).forEach((joint?: ValidJoints) => {
        ...
        jointInLimit[0] = status[joint]['position'] - eps >= status[joint]['lower_limit']; // Lower joint limit
        jointInLimit[1] = status[joint]['position'] + eps <= status[joint]['upper_limit']; // Upper joint limit
...
export function statustoInCollision(status): ValidJointStateDict {
    ...
    Object.keys(status).forEach((joint?: ValidJoints) => {
        ...
        if (joint === "mobile_base" || joint === "other" || joint === "joint_head_tilt" || joint === "joint_head_pan" || joint === "joint_wrist_roll" || joint === "joint_wrist_pitch" || joint === "joint_wrist_yaw" || joint === "joint_gripper") {
            inCollision[joint] = jointInCollision;
        } else {
            jointInCollision[0] = status[joint]['effort'] < JOINT_EFFORT_LIMITS[joint][0]; // Lower effort limit
            jointInCollision[1] = status[joint]['effort'] > JOINT_EFFORT_LIMITS[joint][1]; // Upper effort limit
            inCollision[joint] = jointInCollision;
        }
    });
    return inCollision
